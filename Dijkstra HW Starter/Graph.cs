...
namespace Dijkstra_HW_Starter
{
	class Graph
	{
		...
		{
            //Resetting each vertex and setting the SourceDistance of the starting vertex to zero
            ResetPaths();
            v.SourceDistance = 0;
            v.Permanent = true;

            //Creating current variable and setting it to the initial vertex
            Vertex current = v;

            //Creating an integer that represent the amount of permanent vertexes and looping the following code until all vertexes are permanent
            int numPerm = 1;
            while(numPerm < vertices.Count)
            {
                //Using the FindAdjacent method to create a list of adjacent, non-permanent vertices to the current vertex
                List<Vertex> adjacent = FindAdjacent(current);

                //Finding the shortest distance to all vertices in the adjanect list
                foreach (Vertex vert in adjacent)
                {
                    //Calculating the distance from the source vertex to the current vertex
                    int distance = current.SourceDistance + FindDistance(current, vert);

                    //If the calculated distance is less than the current vertex's established SourceDistance,
                    //  the calculated distance is assigned to the SourceDistance and their PreviousVertex field is set to current
                    if (distance < vert.SourceDistance)
                    {
                        vert.SourceDistance = distance;
                        vert.PreviousVertex = current;
                    }
                }

                //Setting the current variable to a nonpermanent vertext which has the smallest SourceDistance value
                int smallest = int.MaxValue;
                foreach(Vertex vert in vertices)
                {
                    if(vert.SourceDistance < smallest && vert.Permanent != true)
                    {
                        smallest = vert.SourceDistance;
                        current = vert;
                    }
                }

                //Setting the current variable's Permanent field to true and increasing the value of numPerm
        ...
        //FindDistance Helper Method
        public int FindDistance(Vertex vert1, Vertex vert2)
        {
            //Finding the indexes of each both vertices
            int v1 = vertNameToIndex[vert1.Name];
            int v2 = vertNameToIndex[vert2.Name];

            //Returns the distance stored in the adjacency matrix
            return adjMatrix[v1, v2];
        }

        //FindAdjacent Helper Method
        public List<Vertex> FindAdjacent(Vertex vert)
        {
            //Creating an adjacency list
            List<Vertex> adjacent = new List<Vertex>();

            //Finding the index of the vertex
            int v = vertNameToIndex[vert.Name];

            //Finding the indexes of all adjacent vertices
            for(int i = 0; i < MaxVertices; i++)
            {
                if(adjMatrix[v, i] != 0)
                {
                    //Adding the vertex to the adjacency list if it is not permenant
